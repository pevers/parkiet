...
- `chonker.py`: extracts from a transcript.json file (Output of online service) the transcript and chunks the audio into 30 seconds segment and adds emotion labels (experimental)
- `sampler.py`: from the chunks generated by `chonker.py` we create pairs of segments and single segments that can be viewed in the `viewer.py` app
- `preprocess_audio.py`: preprocesses the audio file and creates chunks using Pyannote so that we can transcribe short segments using our whisper-nl model
