...
def setup_logging(log_dir: Path, worker_id: int, gpu_id: int) -> str:
    ...
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s - Worker-%d-GPU-%d - %(levelname)s - %(message)s" % (worker_id, gpu_id),
        handlers=[
...
def start_worker_process(
    ...
    cmd = [
        sys.executable,
        "-m", "parkiet.audioprep.chunker",
        "worker",
        "--queue-name", queue_name,
        "--whisper-checkpoint-path", whisper_checkpoint_path,
        "--temp-dir", worker_temp_dir,
        "--gpu-id", str(gpu_id),
...
def cleanup_zombie_processes():
    ...
    for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
        try:
            ...
            if (cmdline and 
                'parkiet.audioprep.chunker' in ' '.join(cmdline) and
                proc.info['pid'] != current_pid):
                
                print(f"Found existing chunker process (PID: {proc.info['pid']}), terminating...")
                proc.terminate()
                try:
                    proc.wait(timeout=5)
