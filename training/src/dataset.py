...
from dia.config import DEFAULT_SAMPLE_RATE
...
class AudioPromptDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, Union[torch.Tensor, str]]:
        ...
        waveform, orig_sample_rate = torchaudio.load(
            sample["audio_path"], channels_first=True
        )  # C, T
        if orig_sample_rate != DEFAULT_SAMPLE_RATE:
            waveform = torchaudio.functional.resample(
                waveform, orig_sample_rate, DEFAULT_SAMPLE_RATE
            )
        ...
        if (
            ...
            and waveform.shape[1] > self.max_audio_length
        ):
