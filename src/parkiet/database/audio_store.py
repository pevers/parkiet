...
class AudioStore:
    ...
    def calculate_speaker_stats(self) -> dict[int, "SpeakerStat"]:
        """
        Calculate speaker statistics including duration, sampling probability, and class-balanced weights.

        Returns:
            Dictionary mapping speaker_id to SpeakerStat
        """
        from parkiet.audioprep.arrow_writer import SpeakerStat
        ...
        with self.db.get_cursor() as cursor:
            ...
            # durations
            d = {r["speaker_id"]: float(r["total_duration"]) for r in rows}

            # sampling distribution with temperature alpha and floor tau
            alpha = 0.35
            tau = 1800.0  # 30 minutes in seconds
            raw = {sid: (dur + tau) ** alpha for sid, dur in d.items()}
            Z = sum(raw.values()) or 1.0
            p = {sid: v / Z for sid, v in raw.items()}

            # clamp sampling probs
            p_min, p_max = 5e-5, 5e-2
            p = {sid: min(max(v, p_min), p_max) for sid, v in p.items()}
            # renormalize after clamping
            Z2 = sum(p.values())
            p = {sid: v / Z2 for sid, v in p.items()}

            # class-balanced weights (use minutes as "count unit" to tame exponents)
            beta = 0.9999
            unit = 60.0  # seconds per "count"

            def cb(dur):
                n = max(dur / unit, 1.0)
                return (1.0 - beta) / (1.0 - (beta**n))

            w = {sid: cb(dur) for sid, dur in d.items()}

            # clamp loss weights
            w_min, w_max = 0.5, 3.0
            w = {sid: min(max(v, w_min), w_max) for sid, v in w.items()}

            return {
                sid: SpeakerStat(
                    duration_sec=d[sid], sample_prob=p[sid], cb_weight=w[sid]
                )
                for sid in d.keys()
            }

    def get_chunk_owner(self, chunk_id: int) -> int | None:
        """
        Get the speaker who speaks the most in this chunk (the chunk owner).

        Args:
            chunk_id: ID of the chunk

        Returns:
            Speaker ID of the chunk owner, or None if no speakers found
        """
        with self.db.get_cursor() as cursor:
            cursor.execute(
                """
                ...
                WHERE chunk_id = %s AND speaker_id IS NOT NULL
                GROUP BY speaker_id
                ORDER BY duration_in_chunk DESC
                LIMIT 1
            """,
                (chunk_id,),
            )
            ...
            result = cursor.fetchone()
            return result["speaker_id"] if result else None
