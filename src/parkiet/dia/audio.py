...
def build_revert_indices(
    ...
    t_idx_unclamped_BxTxC = t_idx_BT1 + delay_arr.view(1, 1, C)

    # Clamped time indices for safe gathering
    t_idx_clamped_BxTxC = torch.minimum(
...
def revert_audio_delay(
    audio_BxTxC: torch.Tensor,
    pad_value: int,
    precomp: tp.Tuple[torch.Tensor, torch.Tensor, torch.Tensor],
    T: int,
) -> torch.Tensor:
    """
    ...
    t_idx_unclamped_BxTxC = t_idx_unclamped_BxTxC.to(device)
    indices_BTCx3 = indices_BTCx3.to(device)
    ...
    result_BxTxC = torch.where(
        t_idx_unclamped_BxTxC >= T_tensor, pad_tensor, gathered_BxTxC
    )
