...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        q_position = q_positions.unsqueeze(-1).unsqueeze(-1)
        q_sinusoid_inp = q_position / self.rotary_emb.timescale
        q_sin = torch.sin(q_sinusoid_inp)
        q_cos = torch.cos(q_sinusoid_inp)

        # Apply RoPE to keys
        kv_position = kv_positions.unsqueeze(-1).unsqueeze(-1)
        kv_sinusoid_inp = kv_position / self.rotary_emb.timescale
        kv_sin = torch.sin(kv_sinusoid_inp)
        kv_cos = torch.cos(kv_sinusoid_inp)
