...
def view_parquet_dataset(
    parquet_path: Path, limit: int = 10, show_audio: bool = False
) -> None:
    ...
    for file_path in parquet_files:
        ...
        for idx, row in sample_df.iterrows():
            ...
            log.info(f"  Speaker Weight: {row['speaker_weight']:.6f}")
        ...
        log.info(
            f"  Speaker weight range: {df['speaker_weight'].min():.3f} - {df['speaker_weight'].max():.3f}"
        )
        log.info(
